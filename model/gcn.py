...
class GCN(nn.Module):
    ...
    def __init__(self, opt, embeddings, mem_dim, num_layers):
        ...
        self.a = nn.Parameter(torch.zeros(size=(2*self.mem_dim, 1)))
    ...
    def forward(self, adj, inputs):
        ...
        for l in range(self.layers):
            xW = self.W[l](gcn_inputs)
            B, N, C = xW.size()
        
            
            a_input = torch.cat([xW.repeat(1, 1, N).view(B, N * N, C), xW.repeat(1, N, 1)], dim=2).view(B,N, N,2 * self.mem_dim)  # [B,N,N,2C]
